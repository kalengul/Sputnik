...
begin
  ...
  if (NewSattelite.Sat.YgolNaklona<>0) and (NewSattelite.Sat.YgolNaklona<>180) and (Tg<=1) then  //Проверка на возможность вычислениея тангенсов
    DugaEkvatora:=ArcSin(Tg)  //Вычисления смещения по дуге экватора
  else
    DugaEkvatora:=0;
...
begin
LaKolSat.Caption:='Кол-во запущенных спутников - '+IntToStr(KolSattelite);
end;
